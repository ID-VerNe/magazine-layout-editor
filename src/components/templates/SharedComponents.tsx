...
export const CoverBadgeLogo: React.FC<{ page: PageData }> = ({ page }) => {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: page.featuredText
          ? page.badgeColor || page.accentColor || '#ccff33'
          : 'transparent',
        padding: page.featuredText ? '4px 12px' : '0px',
        borderRadius: `${page.badgeRadius ?? (page.featuredText ? 15 : 0)}px`,
        boxShadow: page.featuredText
...
export const ImageFrame: React.FC<{ page: PageData; defaultHeight: number }> = ({ page, defaultHeight }) => {
  const { height, transform, objectPosition } = getImagePositionStyles(page.imageConfig, defaultHeight);
...
export const BylineDisplay: React.FC<{ byline?: string; fontFamily: string; className?: string }> = ({
  byline = '',
  fontFamily,
  className,
}) => {
  if (!byline || !byline.includes('|')) {
    return (
      <span className={`italic ${className} whitespace-pre-wrap`} style={{ fontFamily }}>
        {formatMagazineText(byline)}
      </span>
    );
  }
...
export const FooterDisplay: React.FC<{ page: PageData; pageIndex: number; totalPages: number }> = ({
  page,
  pageIndex,
  totalPages,
}) => {
  ...
  const styles =
    layout === 'blueprint'
      ? {
          container: 'text-slate-500 border-t border-slate-300',
          dots: { active: 'bg-slate-800', inactive: 'bg-slate-200' },
        }
      : layout === 'typography'
      ? {
          container: `${isDark ? 'text-white/40' : 'text-slate-400'} border-t ${
            isDark ? 'border-white/10' : 'border-slate-200'
          }`,
          dots: {
            active: isDark ? 'bg-white/60' : 'bg-slate-600',
            inactive: isDark ? 'bg-white/10' : 'bg-slate-200',
          },
        }
      : {
          container: 'text-neutral-500 border-t border-neutral-200',
      ...
      {Array.from({ length: totalPages }).map((_, i) => (
        <div
          ...
          className={`w-2 h-2 rounded-full transition-all duration-300 ${
            i === pageIndex ? `scale-125 ${styles.dots.active}` : styles.dots.inactive
          }`}
        />
      ))}
    </div>
