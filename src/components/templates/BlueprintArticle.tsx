...
export default function BlueprintArticle({ page, pageIndex, totalPages }: TemplateProps) {
  ...
  const blueprintHeader = (
    <div className="px-10 border-b-2 border-slate-800 pb-4 mb-8 flex justify-between items-end text-slate-900">
      <div className="space-y-2 max-w-[70%]">
        <span className="inline-block text-[10px] font-bold bg-[#ff6600] text-white px-2 py-0.5 shadow-sm uppercase">
          Technical_Spec // bilingual
        </span>
        <AutoFitHeadline
          as="h1"
          text={page.titleEn || ''}
      ...
      <div className="text-right text-[9px] font-black whitespace-pre-wrap">
        ...
        <div className="text-slate-500 mt-1 uppercase">
          Authorized: {formatMagazineText(page.byline?.split('|')[0] || 'UNKNOWN')}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full h-full relative font-mono text-slate-800 overflow-hidden flex flex-col">
      {/* Grid Background */}
      <div
        ...
        <div className="flex flex-col">
          {page.paragraphs?.map((p, index) => {
            ...
            return (
              ...
              >
                {/* English Side - Fix Tailwind dynamic class */}
                <div
                  className={`space-y-2 ${
                    isFirst ? 'border-l-4 border-[#ff6600]' : 'border-l border-slate-300'
                  } pl-6 py-1`}
                >
                  <span
                    className={`text-[8px] font-black ${
                      isFirst ? 'text-[#ff6600]' : 'text-slate-400'
                    } block mb-1 tracking-widest uppercase`}
                  >
                    Part_{index + 1}.en
                  </span>
                  <p
