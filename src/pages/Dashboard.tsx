...
import { Plus, Clock, FileText, Trash2, Download, Copy, Github } from 'lucide-react';
import { saveProject, deleteProjectData, getProject } from '../utils/db';
import { useUI } from '../context/UIContext';
import { motion, AnimatePresence } from 'framer-motion';
import { TEMPLATES } from '../config/templates';
import { parseProjectFile } from '../utils/projectSerializer';
...
function useGridColumns() {
  const [columns, setColumns] = useState<number>(getResponsiveColumns);
...
export default function Dashboard() {
  ...
  const handleImportProject = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert('Import Error', err.message || 'Failed to import project.');
    } finally {
    ...
    <div className="min-h-screen bg-[#F8F9FC] font-sans text-slate-800 pb-20">
      <nav className="h-16 bg-white border-b border-slate-200 flex items-center justify-between sticky top-0 z-50 pr-8 shadow-sm">
        <div className="flex items-center">
          <div className="w-16 h-16 flex items-center justify-center">
            <div className="w-10 h-10 bg-[#264376] rounded-xl flex items-center justify-center text-white font-black shadow-lg shadow-[#264376]/20 text-sm">
              M
            </div>
          </div>
      ...
      <main className="w-[94%] mx-auto py-6 space-y-10">
        ...
        <section>
            ...
            <div className="flex items-center gap-4">
              {recentProjects.length > columns && (
                <button
                  ...
                  className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-[#264376] transition-all"
                >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-[#264376] hover:bg-slate-50 transition-all shadow-sm active:scale-95"
              >
          ...
          <div
            className={`transition-all duration-500 overflow-hidden ${
              !isRecentExpanded ? 'max-h-[410px]' : 'max-h-[2000px]'
            }`}
          >
            {recentProjects.length > 0 ? (
              <motion.div
                layout
                className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 pb-4"
              >
                      ...
                      <div className="h-[300px] bg-slate-50 relative border-b border-slate-50 flex items-center justify-center overflow-hidden">
                        ...
                        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all z-30">
                          <button
                            ...
                            className="p-2 bg-white/90 backdrop-blur-md text-slate-400 hover:text-red-500 rounded-xl shadow-lg"
                          >
                            <Trash2 size={16} />
                          </button>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              duplicateProject(project.id, e);
                            }}
                            className="p-2 bg-white/90 backdrop-blur-md text-slate-400 hover:text-[#264376] rounded-xl shadow-lg"
                          >
                            <Copy size={16} />
                          </button>
                        </div>
                        <div className="absolute inset-0 bg-[#264376]/0 group-hover:bg-[#264376]/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                          <span className="bg-[#264376] text-white px-5 py-2 rounded-full shadow-xl text-[10px] font-black uppercase tracking-widest scale-90 group-hover:scale-100 transition-transform">
                            Resume
                          </span>
                        </div>
                      </div>
                      <div className="p-5 flex-1 flex flex-col justify-center">
                        <h3 className="font-black text-xs uppercase tracking-tight text-slate-900 line-clamp-1 mb-1">
                          {project.title}
                        </h3>
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[8px] font-black text-[#264376] uppercase px-1.5 py-0.5 bg-[#264376]/5 rounded-md">
                            {project.type}
                          </span>
                        </div>
                      </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            {TEMPLATES.length > columns && (
              <button
                ...
                className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#264376] bg-white border border-slate-200 rounded-xl hover:border-[#264376] transition-all shadow-sm active:scale-95"
              >
          ...
          <div
            className={`transition-all duration-500 overflow-hidden ${
              !isTemplatesExpanded ? 'max-h-[410px]' : 'max-h-[2000px]'
            }`}
          >
            <motion.div
              layout
                  ...
                  >
                    <div className="h-[300px] bg-white relative border-b border-slate-50 flex items-center justify-center overflow-hidden">
                      <div className="absolute top-4 left-4 z-20">
                        <span
                          className={`px-2 py-1 rounded-[4px] text-[8px] font-black uppercase tracking-widest shadow-sm ${
                        ...
                        <img
                          ...
                          onError={(e) => {
                            (e.target as HTMLImageElement).src =
                              'https://via.placeholder.com/400x565?text=Template';
                          }}
                        />
                      </div>
                      <div className="absolute inset-0 bg-[#264376]/0 group-hover:bg-[#264376]/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <span className="bg-[#264376] text-white px-6 py-2.5 rounded-full shadow-2xl text-[10px] font-black uppercase tracking-widest">
                          Select
                        </span>
                      </div>
                    </div>
                    <div className="p-5 flex-1 flex flex-col justify-center">
                      <h3 className="font-black text-xs uppercase tracking-widest text-slate-900 mb-1">
                        {template.name}
                      </h3>
                      <p className="text-[10px] text-slate-400 leading-relaxed line-clamp-2 font-medium italic">
